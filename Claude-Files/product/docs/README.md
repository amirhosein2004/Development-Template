...
2. **Fill in the placeholders** in `CLAUDE.md`, `README.md`, and `AUDIT_PROMPT.md`:
   - `GotoRole` — human-readable product name (e.g. `Heptapeak`, `Soradis`, `Avakav`).
   - `gotorole` — short slug used in paths (e.g. `heptapeak`, `soradis`, `avakav`).
   - `POSITIONING` (see `CLAUDE.md`) — one-paragraph description of what the product is and who it's for. Drives everything downstream.
   - `iranian-language-learning, global-language-learning, regional-mena, adjacent-learning-tools` — the market segments you group competitors under (e.g. Iranian IT services, global consultancies, cloud/hosting, headless CMS, SaaS publishing, ...). Rename or add rows in `AUDIT_PROMPT.md` to match.
   ...
   - locale rules — if the product is Persian-first (RTL, Persian numerals, self-hosted fonts, sanctions-safe assets), keep those hard rules in `CLAUDE.md`. Otherwise strip them.
