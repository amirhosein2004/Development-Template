---
description: workspace — <status|sync|clean|mrs> <touched|all>. status — read-only snapshot. sync — ff-pull each repo's default branch. clean — prune merged worktrees/branches/remotes. mrs — list open MRs.
argument-hint: <status|sync|clean|mrs> <touched|all>
---
...
```
/workspace status <touched|all>   # per-repo table: branch, clean/dirty, ahead/behind, MR count, stash count, verdict
/workspace sync   <touched|all>   # checkout default branch and ff-pull each repo — remote-read-only
/workspace clean  <touched|all>   # prune merged worktrees/branches/remote refs — local-only
/workspace mrs    <touched|all>   # list every open MR on origin via glab, grouped by repo
```
...
The candidate set is every discovered repo — the docs container at the workspace top level plus the three discipline docs (`business/docs`, `product/docs`, `tech/docs`) plus every subdirectory of `tech/` whose `.git` exists (directory or file — worktrees count), plus **optional** workspace-tooling repos under `scripts/` (see `scripts/` handling below).

**`scripts/` handling** (optional workspace-tooling folder — bootstrap / initializer / cross-repo maintenance scripts; not every project ships one — always probe):

- If `<workspace-root>/scripts/` does not exist → skip.
- If `<workspace-root>/scripts/.git` exists (directory or file) → `scripts/` is itself a single git repo. Treat as one repo whose workspace-relative path is `scripts`.
- Otherwise, if `<workspace-root>/scripts/*/` contains subdirectories with their own `.git` → each is an independent repo. Enumerate every immediate subdirectory whose `.git` exists; each is a repo whose workspace-relative path is `scripts/<name>` (e.g. `scripts/initializer`).
- Both layouts can coexist in principle; enumerate whichever `.git` entries actually exist.

Scripts repos participate in every sub-op (`status` / `sync` / `clean` / `mrs`) exactly like any other repo — same default-branch resolution, same git operations.
